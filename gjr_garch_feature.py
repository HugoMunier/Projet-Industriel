"""
gjr_garch_feature.py
--------------------
Fonctions utilitaires pour le calcul de la volatilité conditionnelle GJR-GARCH(1,1),
utilisée comme variable d'entrée du classificateur LSTM de régimes de stress.

Le modèle GJR-GARCH (Glosten-Jagannathan-Runkle) étend le GARCH standard en
ajoutant un terme asymétrique qui amplifie la volatilité après les chocs négatifs,
capturant l'effet de levier bien documenté sur les marchés actions.
"""
...
def compute_gjr_garch_vol(
    ...
    """
    Ajuste un modèle GJR-GARCH(p, o, q) et retourne la volatilité conditionnelle.
    ...
    o : int
        Ordre des termes asymétriques (effet de levier). Par défaut 1.
    q : int
    ...
    model = arch_model(returns_scaled, vol="Garch", p=p, o=o, q=q, dist=dist)
    result = model.fit(disp="off")
    cond_vol = result.conditional_volatility / 100
    return cond_vol


def get_gjr_garch_features(df: pd.DataFrame) -> pd.Series:
    """
    Calcule la volatilité conditionnelle GJR-GARCH(1,1) à partir d'un DataFrame de prix.
    ...
    vol_series = compute_gjr_garch_vol(valid_returns)
